...
#### 집계 테이블 
카운팅을 위해 필드를 추가한 경우 (`likeCount`) 어떤 방식이든 카운팅이 제대로 되도록 락을 잡아야 한다. 락을 잡는다는 것은 쓰기 성능에는 치명적일 수 있다.

때때로 쓰기 성능과 읽기 성능을 트레이드 오프 시키는 방법을 선택할 수 있다.

카운팅을 위한 테이블을 하나 만들고 집계해야하는 데이터를 row 로 쭉 쌓는다.
이후 조회시에는 카운트 쿼리를 사용한다.

읽기 성능은 상당히 안 좋아진 것 같다.<br/>
목록조회시 매 row 마다 카운트 쿼리가 날라가야 한다.<br/>
but, 쓰기시에는 아무런 추가적인 조치가 없다. 그냥 insert 뿐이다. <br/>
