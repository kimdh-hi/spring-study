...
### cursor based
- JDBC ResultSet 메커니즘 사용
- 다음 데이터 호출시 커서를 옮겨가며 데이터를 반환
- DB connection 후 배치처리가 완료될 때까지 데이터를 읽으므로 DB 와의 SocketTimeout 시간을 충분히 크게 설정할 필요 있음
- 모든 데이터 반환 결과를 메모리에 유지하므로 메모리 사용량이 비교적 많음

### paging based
- 페이징 단위(page size)로 데이터 조회 (offset, limit)
- 한 페이지를 읽을 때마다 db connection 을 끊으므로 SocketTimeout 위험이 거의 없음
- 페이징 단위의 결과만 매모리에서 처리하므로 메모리 사용량을 비교적 적음
