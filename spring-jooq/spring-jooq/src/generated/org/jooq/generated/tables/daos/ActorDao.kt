...
open class ActorDao(configuration: Configuration?) : DAOImpl<ActorRecord, Actor, Long>(JActor.ACTOR, Actor::class.java, configuration) {
     ...
     * upperInclusive</code>
     */
    fun fetchRangeOfJActorId(lowerInclusive: Long?, upperInclusive: Long?): List<Actor> = fetchRange(JActor.ACTOR.ACTOR_ID, lowerInclusive, upperInclusive)

    /**
     * Fetch records that have <code>actor_id IN (values)</code>
     */
    fun fetchByJActorId(vararg values: Long): List<Actor> = fetch(JActor.ACTOR.ACTOR_ID, *values.toTypedArray())

    /**
     * Fetch a unique record that has <code>actor_id = value</code>
     */
    fun fetchOneByJActorId(value: Long): Actor? = fetchOne(JActor.ACTOR.ACTOR_ID, value)
